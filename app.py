    ...
    try:
        ...
        config = {
            ...
            'roboflow': {
                ...
                'model_id': os.getenv('ROBOFLOW_MODEL_ID', 'custom-workflow-object-detection-tgnqc-af4gr/1')
            },
            'google_maps': {
...
class SolarAnalysisSystem:
    ...
    def _load_history(self):
        """Load history from disk, return empty list if not found."""
        try:
    ...
    def save_to_history(self, type, user_id, data):
        record = {
            "id": f"HIST_{int(datetime.now().timestamp() * 1000)}_{random.randint(100, 999)}",
            "type": type,  # 'analysis' or 'subsidy'
            "user_id": user_id,
    ...
    def get_history(self, user_id, is_admin=False):
        if is_admin:
            return sorted(self.history, key=lambda x: x['created_at'], reverse=True)
        return sorted([h for h in self.history if h.get('user_id') == user_id], key=lambda x: x['created_at'], reverse=True)

    def delete_history(self, record_id, user_id, is_admin=False):
        for i, record in enumerate(self.history):
            if record['id'] == record_id:
                if is_admin or record.get('user_id') == user_id:
                    del self.history[i]
                    self._save_history()
                    return True
        return False
...
def save_analysis():
    ...
    try:
        ...
        sample_id = data.get('sample_id', f"TEST_{random.randint(100, 999)}")
        filename = f"{sample_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        try:
            ...
            logger.warning(f"Failed to save to file, storing only in memory: {e}")
            filepath = "memory_only"
        
        # Add to history
        record = solar_system.save_to_history('analysis', user_id, data)
        solar_system.analysis_results[sample_id] = dict(data)
...
@app.route('/api/subsidy/save', methods=['POST'])
def save_subsidy():
    """Save subsidy results to history"""
    try:
        data = request.json
        user_id = data.get('user_id', 'anonymous')
        record = solar_system.save_to_history('subsidy', user_id, data)
        return jsonify({
            "message": "Subsidy saved successfully",
            "history_record": record
        })
    except Exception as e:
        logger.error(f"Save subsidy error: {str(e)}")
        return jsonify({"error": str(e)}), 500

@app.route('/api/history', methods=['GET'])
def get_user_history():
    """Get history for User/Admin"""
    try:
        user_id = request.args.get('user_id', 'anonymous')
        is_admin = request.args.get('is_admin', 'false').lower() == 'true'
        history = solar_system.get_history(user_id, is_admin)
...
def delete_history_record(record_id):
    try:
        user_id = request.args.get('user_id', 'anonymous')
        is_admin = request.args.get('is_admin', 'false').lower() == 'true'
        success = solar_system.delete_history(record_id, user_id, is_admin)
        if success:
...
def sms_subscribe():
    """Placeholder SMS notification subscription endpoint.
    Replace this with a real SMS provider (e.g. Twilio, MSG91, AWS SNS).
    """
    try:
        ...
        # ---- PLACEHOLDER ----
        # Replace with real SMS API call, e.g.:
        # client = TwilioClient(account_sid, auth_token)
        # client.messages.create(body='Surya Sys: Notifications enabled!', from_='+1234567890', to=phone)
        # ---------------------
        
        logger.info(f"[SMS Placeholder] Subscribe request: email={email}, phone={phone}")
        return jsonify({
            "success": True,
            "message": "SMS subscription recorded (placeholder). Integrate a real SMS provider to activate.",
            "email": email
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
